...
window.angular.module('myApp', ['afkl.lazyImage', 'infinite-scroll', 'sticky', 'matchMedia'])

.controller('ApplicationController', ['$scope', '$http', '$window', 'screenSize',
function($scope, $http, $window, screenSize) {
    $scope.displayed = [];
    $scope.count = 0;
    ...
    screenSize.rules = {
        mportrait: '(max-width: 450px)',
        mlandscape: '(max-width: 750px)'
    };
    
    if (screenSize.is('mportrait')) {
        $scope.thumbCount = 3;
    } else if (screenSize.is('mlandscape')) {
        $scope.thumbCount = 6;
    } else {
        $scope.thumbCount = 10;
    }
